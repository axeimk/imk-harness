...
一番大事な持ち物は個々の設定ではなく、プロジェクトスコープの育て方の規約と、
それを実行する harness-check スキルの 2 つ。
...
- **ユーザースコープ（このリポジトリ）は最小限に保つ。** 個人的な好み、共通 permissions、そして「プロジェクトスコープの育て方」の規約だけを持つ。
...
1. プロジェクトスコープの現状確認 — CLAUDE.md / AGENTS.md、verify / tdd スキル、CONTEXT.md（用語集）、
