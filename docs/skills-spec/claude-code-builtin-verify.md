# Claude Code ビルトイン verify スキルの調査

- 調査日: 2026-07-13
- 一次情報: Claude Code v2.1.207 のバイナリ（単一 Mach-O 実行ファイル）に埋め込まれた
  SKILL.md 本文を抽出したもの（公式ドキュメントに本文の記載なし）
- 目的: プロジェクト verify スキルを設計する際、ビルトインとの名前の重なり・役割分担を
  正しく理解するための判断材料

## 要点

### ビルトインとプロジェクト verify スキルは「同名で共存する」設計

名前の重なりは衝突ではなく、ビルトインが前提とする統合ポイント。

- **ビルトイン verify** = 検証の**プロトコル**（汎用）。diff の範囲特定 → surface の特定 →
  起動・操作 → 逸脱入力での probe → PASS/FAIL/BLOCKED/SKIP の定型レポート、という進め方を定める
- **プロジェクト verify**（`.claude/skills/verify/SKILL.md`）= ビルド・起動・操作の
  **レシピ**（プロジェクト固有）。ビルトインが探しに来て、読む

ビルトインの本文に次の動作が明記されている:

1. まず `.claude/skills/`（リポジトリルートと、diff が触るパッケージ配下の両方）を探す
2. `verifier-*` スキル（surface 別の証跡取得プロトコル）があれば Skill ツールで起動して従う
3. `run-*` スキルがあればビルド・起動の手順として使う
4. どちらも無ければ README 等からコールドスタート（約 15 分でタイムボックス）し、
   成功したら**ビルトイン自身が `.claude/skills/verify/SKILL.md` を作成**して
   レシピを永続化する
5. プロジェクト verify が既にあれば、それが誤っていたときだけ編集する
   （書式の書き換え・再構成はしない）

### プロジェクト verify スキルに書くべき内容（ビルトインが期待する形）

- 実際に動いたビルド・起動・操作のコマンド
- 動かして確認する価値のあるフロー
- ハマりどころ（gotchas）
- 短く保つ。テストコマンドの羅列にしない（ビルトインは「テスト実行・typecheck は
  検証ではない（CI の再実行にすぎない）」と明確に禁止している）

### そのほかの特徴

- 検証は「runtime observation」: 変更コードが実行される所までアプリを動かし、
  観察結果（出力・レスポンス・スクリーンショット）だけを証拠とする
- `import` して関数を直接呼ぶのは「自作ユニットテスト」であり検証ではない、と明記
- 破壊的パス（削除・公開・外部送信）は dry-run や安全なターゲットが無ければ実行しない
- 判定に迷ったら FAIL（false PASS は壊れたコードを出荷し、false FAIL は人間の一瞥で済む）

## 抽出方法（再現手順）

バイナリ内に SKILL.md 本文が JS 文字列リテラルとして埋め込まれている。

```sh
B=~/.local/share/claude/versions/<version>
LC_ALL=C grep -a -b -o 'exercising it end-to-end' "$B"   # バイトオフセットを得る
dd if="$B" bs=1 skip=<offset-手前> count=16000 > raw.txt  # 前後を切り出す
# raw.txt から `---\nname: verify` 〜 `` `;var `` の範囲を取り、
# \uXXXX エスケープとサロゲートペアを解決すると本文になる
```

examples/cli.md と examples/server.md も同様に隣接して埋め込まれている（本調査では本文のみ抽出）。
