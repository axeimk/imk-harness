...
#   - settings.json / config.toml / opencode.json: コピー配置のため削除しない（サマリで案内）
#
# 使い方:
#   ./uninstall.sh              # 確認あり
...
apply_changes() {
  ...
  # コピーで配置したファイルの案内
  local f
  for f in "$HOME/.claude/settings.json" "$HOME/.codex/config.toml" "$OPENCODE_CONFIG_DIR/opencode.json"; do
    if [ -e "$f" ]; then
  ...
  local leftover
  leftover="$(ls -1d "$HOME/.claude/CLAUDE.md".bak.* "$HOME/.codex/AGENTS.md".bak.* "$OPENCODE_CONFIG_DIR/AGENTS.md".bak.* 2>/dev/null || true)"
  if [ -n "$leftover" ]; then
